...
支持绑定多个 PC / Creator 账号，扫码登录、手机验证码、Cookie 导入三种方式。Cookie 加密存储，2 小时自动健康巡检，过期自动通知。海外 [Rednote PC 账号](docs/rednote-pc-account.md) 与中国站账号隔离，可在身份检查通过后作为有限采集账号使用。
...
| 模块 | 功能 |
|------|------|
| **账号矩阵** | 多 PC / Creator 账号绑定、Rednote PC Cookie 导入、Cookie 加密存储、2h 自动健康巡检、身份漂移保护与过期通知 |
| **笔记发现** | 关键词搜索、URL 直查、多维筛选、已保存标记、一键入库 |
| **数据抓取** | 批量 URL / 搜索 / 评论抓取、Excel 导出、素材本地下载；Rednote PC 限公开主页笔记列表和单篇详情，且不自动下载媒体 |
